...
var UpnpControlPoint = function() {
	...
	this.ssdp.on("DeviceFound", function(device) {
		var s = device.location.split("%wlan0:");
		if(s[0] == "http://fe80::9218:7cff:fe42:dd1e")
			device.location = "http://192.168.1.148:"+s[1];
		if(s[0] == "http://fe80::216:d4ff:fec2:c06")
			device.location = "http://192.168.1.103:"+s[1];
...
UpnpControlPoint.prototype._getDeviceDetails = function(udn, location, callback) {
	...
	req.on('socket', function(socket) {
		if(socket.address() != null) {
			...
			console.log("Socket address error!");
			console.log(socket._httpMessage);
		...
		try {
			parser.parseString(reqContent, function(err, result) {
				...
				if (subscription) {
					...
					values = {};
					var properties = result["propertyset"]["property"];
